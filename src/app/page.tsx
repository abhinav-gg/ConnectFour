...
import { CellState, Game } from "@lib/connect4";
  ...
  const renderGridOverlay = (i: number, j: number) => {
    if (j === gameState.lowestCellIndices[i] && gameState.grid[i][j] === CellState.Empty) {
      if (gameState.currentPlayer === 0) {
        return <span
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 group-hover:bg-emerald-300 w-5 aspect-square rounded-full"
        ></span>;
      }
      else {
        return <span
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 group-hover:bg-yellow-300 w-5 aspect-square rounded-full"
        ></span>;
      }
    }
    else if (gameState.lastMove[0] === i && gameState.lastMove[1] === j) {
      return <span
        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-transparent ring-4 sm:ring-8 ring-white rounded-full aspect-square circ-width"
      ></span>;
    }
  };
  ...
  return (
    <main className="w-full h-screen flex flex-row bg-[#302E2B]">
      <div className="h-full bg-neutral-800 hidden md:flex flex-col">
        <h1
          className="flex items-center justify-center w-full text-xl text-white font-bold py-3 px-6"
        >
          Connect 4
        </h1>
      ...
      <div className="bg-[#302E2B] flex md:p-4 items-center justify-center h-screen">
        <div className="flex flex-col w-full justify-start gap-y-3 max-h-screen">
          <div className="flex flex-row gap-2 items-center">
            ...
            >Example Player <span className="font-light text-stone-400">(1600)</span></span>
          </div>
          <div
            className="flex items-center justify-center md:rounded-md flex-grow overflow-hidden relative"
          >
            <div
              className="absolute top-0 left-0 w-full h-full flex flex-row z-10 board-padding"
            >
              {Array.from({ length: 7 }).map((_, i) => (
                <table
                  className="w-full h-full border-spacing-0 border-none group"
                  cellSpacing="0"
                  cellPadding="0"
                  key={i}
                >
            ...
            <div
              className="absolute top-0 left-0 w-full h-full p-[1.5060241%] flex flex-row z-20"
            >
              {Array.from({ length: 7 }).map((_, i) => (
                <table
                  className="w-full h-full border-spacing-0 border-none group"
                ...
                >
                  <tbody>
                    {Array.from({ length: 6 }).map((_, j) => (
                      <tr className="w-full h-1/6" key={j}>
                        <td
                          className="w-1/7 h-1/6 group-hover:bg-black/20 cursor-pointer relative"
                          onClick={() => { game.captureCell(i); }}
                        >
          ...
          <div className="flex flex-row gap-2 items-center">
            ...
            <span className="font-medium text-white"
            >You <span className="font-light text-stone-400">(1600)</span></span>
          </div>
        </div>
      </div>
